...
class DoctorReport:
    ...
    def _check_project_config(self) -> list[CheckResult]:
        ...
        if not presets_file.exists():
            results.append(CheckResult(
                "CMakePresets.json", CheckStatus.WARN,
                message="Not found (generated by conan install)",
                fix_hint="Run 'sbuild configure' to generate CMake presets via Conan",
            ))
        else:
            results.extend(self._check_cmake_presets(presets_file))

        return results

    def _check_cmake_presets(self, presets_file: Path) -> list[CheckResult]:
        results: list[CheckResult] = []
        try:
        ...
        # Collect all preset names (including from "include" files)
        preset_names = _collect_preset_names(data)
